...
public class Piece {
	...
	public Piece(TPoint[] points) {
		...
		for(int i = 0; i < body.length; i++){
			mnx = Math.min(mnx, body[i].x);
			mxx = Math.max(mxx, body[i].x);
			mny = Math.min(mny, body[i].y);
			mxy = Math.max(mxy, body[i].y);
		}
		width = mxx - mnx + 1;
		height = mxy - mny + 1;
		Arrays.sort(body);
		...
		for(int i = 0 ; i < body.length; i++){
			temp[body[i].x - mnx] = Math.min(temp[body[i].x - mnx], body[i].y);
		}
		skirt = temp;
	}
	...
	public Piece computeNextRotation() {
		TPoint[] bodyR = new TPoint[body.length];
		for(int i = 0; i < body.length; i++){
			int xR = body[i].x;
			int yR = body[i].y;
			bodyR[i] = new TPoint(this.getHeight() - 1 - yR, xR);
		}
		return new Piece(bodyR);
	}
	...
	public boolean equals(Object obj) {
		...
		if(other.getBody().length != this.getBody().length) return false;
		for(int i = 0; i < this.getBody().length; i++){
			if(!other.getBody()[i].equals(this.getBody()[i])) return false;
		}
		return true;
	}
