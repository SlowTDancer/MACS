...
public class FacePamphletDatabase implements FacePamphletConstants {
	...
	public void deleteProfile(String name) {
		if (profileDataBase.containsKey(name)) {
			FacePamphletProfile profile = profileDataBase.get(name);
			profileDataBase.remove(name, profile);
			Iterator<String> it = profile.getFriends();
			while (it.hasNext()) {
				FacePamphletProfile othersProfile = profileDataBase.get(it.next());
				othersProfile.removeFriend(name);
				it.remove();
			}
		}
	}
	 ...
	 * instance variables.
	 */
	private HashMap<String, FacePamphletProfile> profileDataBase = new HashMap<>();
}
