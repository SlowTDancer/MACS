...
void get_text(map<string, Definition> &grammar, string key){
  Definition definition_now = grammar[key];
  Production product_now = definition_now.getRandomProduction();
  Production::iterator it = product_now.begin();
  for(it; it != product_now.end(); it++){
    string word_now = *it;
    if(grammar.count(word_now)){
      get_text(grammar, word_now);
      continue;
    }
    if(word_now[0] != '<'){
      cout << word_now << " ";
