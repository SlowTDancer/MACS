...
Report_DoReport(Bank *bank, int workerNum)
{
  ...
  assert(rpt); Y;
  sem_wait(&bank->last_worker);
  bank->busy_workers--;
  if(bank->busy_workers != 0) {
    sem_post(&bank->last_worker);
    sem_wait(&bank->tomorrows_work);
    sem_post(&bank->fired_up);
    return 0;
  }
  if (rpt->numReports >= MAX_NUM_REPORTS) {
    for(int i = 0; i < numWorkers - 1; i++){
      sem_post(&bank->tomorrows_work);
    }
    for(int i = 0; i < numWorkers - 1; i++){
      sem_wait(&bank->fired_up);
    }
