...
(define (intersection-points lst)
  (if (null? (cdr lst)) 
    '()
    (append (intersect (car lst) (cadr lst)) 
    (intersection-points (cons (car lst) (cddr lst) ))
    (intersection-points (cons (cadr lst) (cddr lst) )))
  )
)


(define (distance-product haru lst) 
  (apply * (map (lambda (x) 
    (if(equal? haru x)
      1
      (dist haru x))
...
(define (clumped-points lst) 
  (map cadr (prefix-of-list (sort-points (rate-points lst)) (/ (length (sort-points (rate-points lst) ) ) 2) )
  )
)


(define (average lst a b len)
  (if (null? lst)
    (make-pt (/ a len) (/ b len))
    (average (cdr lst) (+ a (x (car lst) ) ) (+ b (y (car lst) ) ) len)
  )
)


(define (average-point lst)
  (list (distance-product (average lst 0 0 (length lst) ) lst) (average lst 0 0 (length lst) ) )
)
...
(define (where-am-i dists stars) 
  (prefix-of-list (sort-points (map best-estimate (all-guesses dists stars) ) ) 6)
)
;;; --------------------------------------------------------------------------------------
;;; 				!!!!!!! TESTING PART !!!!!!!
